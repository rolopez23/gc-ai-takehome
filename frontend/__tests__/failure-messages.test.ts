...
import { FAILURE_MESSAGES, getFailureMessage } from '@/app/evaluate-contract/failure-messages';

describe('FAILURE_MESSAGES', () => {
  test('all 5 failure codes have entries', () => {
    const expectedCodes = ['too_large', 'timeout', 'anthropic_error', 'parse_error', 'unknown'];
  ...
  test('known code returns correct message', () => {
    expect(getFailureMessage('timeout')).toBe('The evaluation timed out. Please try again.');
  });
